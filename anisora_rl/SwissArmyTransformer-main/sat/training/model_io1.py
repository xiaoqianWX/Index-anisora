...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None):
    ...
    ######## shape mismatched supported
    new_state_dict = {}
    for k, v in module.state_dict().items():
        try:
            # assert "quantizer" not in k
            # print("load", k)
            assert sd['module'][k].shape == v.shape, (sd['module'][k].shape, v.shape)
            new_state_dict[k] = sd['module'][k]
        except:
            traceback.print_exc()
            # print("error, %s is not in the checkpoint" % k)#shape不对也会，比如text_embedding当cleaner修改时
            if(k in sd['module']):
                print(sd['module'][k].shape,v.shape,k)
            else:
                print(v.shape,k)###模型多的，参数少的
            new_state_dict[k] = v
    # import os
    # os._exit(23333)
