    ...
    def shared_step(self, batch: Dict) -> Any:
        ...
        if self.noised_image_input:
            ...
            if not self.is_training_from_vae_encoded_data:
                # image = self.encode_first_stage(x[:, :, 0:1].permute(0, 2, 1, 3, 4), batch).permute(0, 2, 1, 3, 4)
                # image_last = self.encode_first_stage(x[:, :, -2:-1].permute(0, 2, 1, 3, 4), batch).permute(0, 2, 1, 3, 4)
                index_ = np.random.randint(x.shape[2]-2)+1
                if np.random.randint(2)>0:
                    image = self.encode_first_stage(x[:, :, index_:index_+1].permute(0, 2, 1, 3, 4), batch).permute(0, 2, 1, 3, 4)
                else:
                    image =  torch.zeros_like(x[:, :, 0:1])
                if np.random.randint(2)>0:
                    image_first = self.encode_first_stage(x[:, :, 0:1].permute(0, 2, 1, 3, 4), batch).permute(0, 2, 1, 3, 4)
                else:
                    image_first =  torch.zeros_like(x[:, :, 0:1])
                if np.random.randint(2)>0:
                    image_last = self.encode_first_stage(x[:, :, -2:-1].permute(0, 2, 1, 3, 4), batch).permute(0, 2, 1, 3, 4)
                else:
                    image_last =  torch.zeros_like(x[:, :, 0:1])
            else:
                # image=self.scale_factor*x[:, :, 0:1] # x: b c t h w
                ...
                if np.random.randint(2)>0:
                    ...
                    image =  torch.zeros_like(x[:, :, 0:1])
                if np.random.randint(2)>0:
                    image_first=self.scale_factor*x[:, :, 0:1]
                else:
                    image_first =  torch.zeros_like(x[:, :, 0:1])
                if np.random.randint(2)>0:
                    image_last=self.scale_factor*x[:, :, -2:-1]
        ...
        if self.noised_image_input:
            ...
            image_first = image_first.permute(0, 2, 1, 3, 4).contiguous()#
            ...
            if self.noised_image_all_concat:
                ...
                fe_0 = torch.zeros_like(x[:, 1:index_])
                fe_1 = torch.zeros_like(x[:, index_+1:-1])
                image = torch.concat([image_first, fe_0, image, fe_1, image_last], dim=1)
